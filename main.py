...
#Sinal do comando  -- Letra minuscula ou maiúscula não faz diferença
...
@client.event
async def in_ready():
    print("O pai ta on como {0.user}".format(client)) #mostra quando ta ligado

@client.command()
async def hello(ctx): #ctx = contexto
    await ctx.send (f"Olá, parça {ctx.author}")
